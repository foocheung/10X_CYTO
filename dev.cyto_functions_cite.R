...
for(i in 1:length(lanes)){
  ...
  SeuratObj[[i]] <- CreateSeuratObject(counts = B1_US_data[[i]], assay = "RNA",min.feature = 20)
...
B1_US_merge <- PercentageFeatureSet(B1_US_merge, pattern = "^MT-",col.name = "percent.mito")
 # B1_US_merge <- AddMetaData(object = B1_US_merge, metadata = Matrix::colSums(B1_US_merge[mito.genes,])/Matrix::colSums(B1_US_merge), col.name = "percent.mito")
  ...
  B1_US_merge2<- JoinLayers(B1_US_merge, overwrite = TRUE)  

  B1_US_merge2 <- B1_US_merge2 %>%
    dplyr::filter(nFeature_RNA > 200 & nFeature_RNA < 3000 & nCount_RNA < 20000 & percent.mito < 30)
  
  ggsave(paste0(save_name,"_postfiltering.pdf"), 
         plot =VlnPlot(B1_US_merge2, c( "percent.mito","percent_ribo","percent_hb", "nFeature_RNA", "nCount_RNA"), 
                       group.by = "Lane", pt.size=0, ncol = 3)) + theme(axis.text.x = element_text(angle = 45, hjust=1))
